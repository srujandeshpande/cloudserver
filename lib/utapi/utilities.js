...
function pushMetricV2(action, log, metricObj) {
    const {
        canonicalID,
        authInfo,
        location,
    ...
    const { accountId, userId } = authInfo ? evalAuthInfo(authInfo, canonicalID, action) : {};

    const utapiObj = {
        account: authInfo ? accountId : canonicalID,
        user: authInfo ? userId : undefined,
        location,
        objectDelta,
        sizeDelta,
        incomingBytes,
    ...
    utapi.pushMetricV2(action, log.getSerializedUids(), utapiObj);
}
